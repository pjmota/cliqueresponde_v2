    ...
    if (companies) {
      companies.map(async c => {
        c.queues.map(async q => {
          ...
          const createSequentialUserIdGenerator: (
            userIds: number[]
          ) => () => number = userIds => {
            let currentIndex = -1;
            return () => {
            ...
            for (const ticket of tickets.filter(e => e.id === ticketId)) {
              ...
              if (!userId && !rotationUsers) {
                ...
                if (tempoPassadoB > updatedAtV) {
                  ...
                  const availableUserIds = rotationUsers
                    ? (() => {
                        ...
                        if (lastSeq !== undefined) {
                          ...
                          const startIndex =
                            orderedUsers.findIndex(
                              e => e.sequence === lastSeq
                            ) + 1;
                          const sortedUsers = [
                            ...orderedUsers.slice(startIndex),
                            ...orderedUsers.slice(0, startIndex)
                          ];
                  ...
                  if (
                    !getNextUserId ||
                    availableUserIds.length !== rotationUsers.length
                  ) {
                    getNextUserId =
                      createSequentialUserIdGenerator(availableUserIds);
                  }
                  ...
                  if (availableUserIds.length > 0) {
                    ...
                    if (
                      ...
                      if (rotationUsers && !ticket.userId) {
                        ...
                        const data: any = rotationUsers.filter(
                          (e: any) => e.userId === randomUserId
                        );
                        const rotationData = {
