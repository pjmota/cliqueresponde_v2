...
import { FormControlLabel, Switch, MenuItem, Typography } from "@material-ui/core";
...
const MessageInputCustom = ({ ticketStatus, ticketId, baseUrl, width, showIntegrationOptions = true}) => {
  const classes = useStyles();
  ...
  const fetchIntegration = async () => {
    ...
    setIntegrations(data.data.queueIntegrations)
  }
  ...
  const handleUploadQuickMessageMedia = async (blob, message) => {
    ...
    try {
      ...
      await api.post(baseUrl ?? `/messages/${ticketId}`, formData);
    } catch (err) {
      toastError(err);
      setLoading(false);
    }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await api.post(baseUrl ?? `/messages/${ticketId}`, message);
    } catch (err) {
      toastError(err);
    }
    ...
    return (
      <Paper square elevation={0} className={classes.mainWrapper}>
        ...
        <div className={classes.newMessageBox}>
          ...
          {showIntegrationOptions && <IntegrationOptions
            disabled={disableOption()}
