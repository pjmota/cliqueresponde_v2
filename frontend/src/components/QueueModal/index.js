...
import { RotationOptions } from "../RotationOptions";
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const rotation = await api.get("/rotations", {
          params: {
            queueId: data.id,
        ...
        setRotationId(rotation?.data?.rotations[0]?.id ?? null)
        setLastSequenceId(rotation?.data?.rotations[0]?.lastSequence ?? null)
        const param = rotation?.data?.rotations[0]?.lastSequence

        setQueue((prevState) => {
          return {
            ...prevState,
            ...data,
            ...(param ? { lastSequence: param } : {})
          };
        });
                  ...
                  <Box marginTop={"1rem"}>
                    ...
                    <div style={{ display: "flex", alignItems: "center",gap: "10px" }}>
                      ...
                      >
                        ...
                        <MenuItem value="-1">Imediato</MenuItem>
                     ...
                     {!queueId && ( <Typography variant="subtitle1" gutterBottom color="textPrimary">
                        {i18n.t("queueModal.form.rotateInfo")}
                      </Typography>)}
                    </div>
                    <div>
                      {showIntegrations && (
                ...
                {tab === 2 && (
                  <>
                    {/* <Typography variant="subtitle1">
                      ...
                      <CustomToolTip
                        title={i18n.t("queueModal.rotation.toolTipTitle")}
                        content={i18n.t("queueModal.rotation.toolTip")}
                      >
                        <HelpOutlineOutlinedIcon
                          style={{ marginLeft: "14px" }}
                          fontSize="small"
                        ...
                        <Grid container spacing={2}>
                          <Grid item sm={6}>
                            <Field
                              ...
                              type="number"
                              label={i18n.t(
                                "queueModal.rotation.form.lastSequence"
                              )}
                              autoFocus
                              error={
                                touched.lastSequence &&
                                Boolean(errors.lastSequence)
                              }
                              helperText={
                                touched.lastSequence && errors.lastSequence
                              }
                              name="lastSequence"
                              variant="outlined"
                              margin="dense"
